...
function Run-Cmd($command, $title) {
    ...
    # Using cmd /c with powershell forward ensures line-buffering is correctly flushed dynamically
    ...
    $Timer.Interval = 100
    $Timer.Add_Tick({
        # Read lines available instantly without breaking UI thread hold
        while (-not $Proc.StandardOutput.EndOfStream) {
            $Line = $Proc.StandardOutput.ReadLine()
            if ($Line -ne $null) {
                $OutBox.AppendText("$Line`r`n")
            }
            # Give UI processing interval window breather room
            [System.Windows.Forms.Application]::DoEvents()
            if ($Proc.StandardOutput.EndOfStream) { break }
        }

        # Check Error Pipes also safely
        ...
        if ($Proc.HasExited) {
            ...
            # Read whatever remains left inside standard reader matrix buffers
